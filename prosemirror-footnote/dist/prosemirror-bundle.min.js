(function (factory) {
  ...
  */
  ...
  function replaceThreeWay$1($from, $start, $end, $to, depth) {
      let openStart = $from.depth > depth && joinable$2($from, $start, depth + 1);
      let openEnd = $to.depth > depth && joinable$2($end, $to, depth + 1);
      let content = [];
  ...
  function replaceTwoWay$1($from, $to, depth) {
      ...
      addRange$1(null, $from, depth, content);
      if ($from.depth > depth) {
          let type = joinable$2($from, $to, depth + 1);
  ...
  let ResolvedPos$1 = class ResolvedPos {
      ...
      */
      static resolveCached(doc, pos) {
          for (let i = 0; i < resolveCache$1.length; i++) {
              let cached = resolveCache$1[i];
              if (cached.pos == pos && cached.doc == doc)
                  return cached;
          }
          let result = resolveCache$1[resolveCachePos$1] = ResolvedPos.resolve(doc, pos);
          resolveCachePos$1 = (resolveCachePos$1 + 1) % resolveCacheSize$1;
          return result;
      }
  };
  let resolveCache$1 = [], resolveCachePos$1 = 0, resolveCacheSize$1 = 12;
  /**
  ...
  */
  ...
  function bindProps$1(obj, self, target) {
      for (let prop in obj) {
          let val = obj[prop];
          if (val instanceof Function)
              val = val.bind(self);
          else if (prop == "handleDOMEvents")
              val = bindProps$1(val, self, {});
          target[prop] = val;
      }
      return target;
  }
  ...
  */
  ...
  active in a state.
  */
  let PluginKey$1 = class PluginKey {
      /**
      Create a plugin key.
      */
  ...
  RopeSequence$1.prototype.append = function append (other) {
    ...
    return (!this.length && other) ||
      (other.length < GOOD_LEAF_SIZE$1 && this.leafAppend(other)) ||
      (this.length < GOOD_LEAF_SIZE$1 && other.leafPrepend(this)) ||
  ...
  RopeSequence$1.from = function from (values) {
    ...
    return values && values.length ? new Leaf$1(values) : RopeSequence$1.empty
  };

  var Leaf$1 = /*@__PURE__*/(function (RopeSequence) {
    ...
    Leaf.prototype.leafAppend = function leafAppend (other) {
      if (this.length + other.length <= GOOD_LEAF_SIZE$1)
  ...
  RopeSequence$1.empty = new Leaf$1([]);

  var Append$1 = /*@__PURE__*/(function (RopeSequence) {
  ...
  const max_empty_items$1 = 500;
  let Branch$1 = class Branch {
      ...
      addTransform(transform, selection, histOptions, preserveItems) {
          ...
          if (overflow > DEPTH_OVERFLOW$1) {
              oldItems = cutOffEvents$1(oldItems, overflow);
              eventCount -= overflow;
          }
          return new Branch(oldItems.append(newItems), eventCount);
      }
      remapping(from, to) {
          let maps = new Mapping$1;
          this.items.forEach((item, i) => {
      ...
      rebased(rebasedTransform, rebasedCount) {
          ...
          if (branch.emptyItemCount() > max_empty_items$1)
      ...
      compress(upto = this.items.length) {
          ...
          return new Branch(RopeSequence$1.from(items.reverse()), events);
      }
  };
  Branch$1.empty = new Branch$1(RopeSequence$1.empty, 0);
  function cutOffEvents$1(items, n) {
      let cutPoint;
  ...
  const DEPTH_OVERFLOW$1 = 20;
  // Apply the latest event from one branch to the document and shift the event
  // onto the other branch.
  function histTransaction$1(history, state, dispatch, redo) {
      let preserveItems = mustPreserveItems$1(state);
      let histOptions = historyKey$1.get(state).spec.config;
      let pop = (redo ? history.undone : history.done).popEvent(state, preserveItems);
      if (!pop)
          return;
      let selection = pop.selection.resolve(pop.transform.doc);
      let added = (redo ? history.done : history.undone).addTransform(pop.transform, state.selection.getBookmark(), histOptions, preserveItems);
      let newHist = new HistoryState$1(redo ? added : pop.remaining, redo ? pop.remaining : added, null, 0, -1);
      dispatch(pop.transform.setSelection(selection).setMeta(historyKey$1, { redo, historyState: newHist }).scrollIntoView());
  }
  let cachedPreserveItems$1 = false, cachedPreserveItemsPlugins$1 = null;
  // Check whether any plugin in the given state has a
  // `historyPreserveItems` property in its spec, in which case we must
  // preserve steps exactly as they came in, so that they can be
  // rebased.
  function mustPreserveItems$1(state) {
      let plugins = state.plugins;
      if (cachedPreserveItemsPlugins$1 != plugins) {
          cachedPreserveItems$1 = false;
          cachedPreserveItemsPlugins$1 = plugins;
          for (let i = 0; i < plugins.length; i++)
              if (plugins[i].spec.historyPreserveItems) {
                  cachedPreserveItems$1 = true;
                  break;
              }
      }
      return cachedPreserveItems$1;
  }
  const historyKey$1 = new PluginKey$1("history");
  new PluginKey$1("closeHistory");
  /**
  A command function that undoes the last change, if any.
  */
  const undo$1 = (state, dispatch) => {
      let hist = historyKey$1.getState(state);
      if (!hist || hist.done.eventCount == 0)
          return false;
      if (dispatch)
          histTransaction$1(hist, state, dispatch, false);
      return true;
  };
  /**
  ...
  */
  ...
  function getIcon$1(root, icon) {
      ...
      if (icon.path) {
          ...
          let name = "pm-icon-" + hashPath$1(path).toString(16);
          if (!doc.getElementById(name))
              buildSVG$1(root, name, icon);
          let svg = node.appendChild(doc.createElementNS(SVG$1, "svg"));
          svg.style.width = (width / height) + "em";
          let use = svg.appendChild(doc.createElementNS(SVG$1, "use"));
  ...
  function buildSVG$1(root, name, data) {
      ...
      let collection = doc.getElementById(prefix$2$1 + "-collection");
      if (!collection) {
      ...
      let sym = doc.createElementNS(SVG$1, "symbol");
      sym.id = name;
      sym.setAttribute("viewBox", "0 0 " + data.width + " " + data.height);
      let path = sym.appendChild(doc.createElementNS(SVG$1, "path"));
  ...
  */
      ...
      render(view) {
          ...
          let dom = spec.render ? spec.render(view)
              : spec.icon ? getIcon$1(view.root, spec.icon)
                  : spec.label ? crelt$1("div", null, translate$1(view, spec.label))
                      : null;
          if (!dom)
  ...
  var mac$6 = typeof navigator != "undefined" && /Mac/.test(navigator.platform);
  ...
  function keyName$1(event) {
    ...
    var ignoreKey = mac$6 && event.metaKey && event.shiftKey && !event.ctrlKey && !event.altKey ||
  ...
  */
  ...
  const mac$5 = typeof navigator != "undefined" ? /Mac|iP(hone|[oa]d)/.test(navigator.platform) : false;
  ...
  function modifiers$1(name, event, shift = true) {
      if (event.altKey)
          name = "Alt-" + name;
      if (event.ctrlKey)
          name = "Ctrl-" + name;
  ...
  */
  function keymap$1(bindings) {
      return new Plugin({ props: { handleKeyDown: keydownHandler$1(bindings) } });
  }
  /**
  ...
  */
  function keydownHandler$1(bindings) {
      ...
      return function (view, event) {
          let name = keyName$1(event), baseName, direct = map[modifiers$1(name, event)];
          ...
          if (name.length == 1 && name != " ") {
              if (event.shiftKey) {
                  ...
                  let noShift = map[modifiers$1(name, event, false)];
              ...
              if ((event.shiftKey || event.altKey || event.metaKey || name.charCodeAt(0) > 127) &&
                  ...
                  // #1060
                  let fromCode = map[modifiers$1(baseName, event)];
  ...
  */
  function exampleSetup(options) {
      let plugins = [
          ...
          keymap$1(buildKeymap(options.schema, options.mapKeys)),
          keymap$1(baseKeymap),
          dropCursor(),
          gapCursor()
      ];
  ...
  */
  function keymap(bindings) {
      return new Plugin$1({ props: { handleKeyDown: keydownHandler(bindings) } });
  }
  /**
  Given a set of bindings (using the same format as
  [`keymap`](https://prosemirror.net/docs/ref/#keymap.keymap)), return a [keydown
  ...
  */
  ...
  menu.insertMenu.content.push(new MenuItem$1({
      title: "Insert footnote",
      label: "Footnote",
      select(state) {
          return insertPoint$1(state.doc, state.selection.from, footnoteSchema.nodes.footnote) != null
      },
      run(state, dispatch) {
          let {empty, $from, $to} = state.selection, content = Fragment$1.empty;
      ...
      open() {
          ...
          this.innerView = new EditorView(tooltip, {
              ...
              state: EditorState.create({
                  doc: this.node,
                  plugins: [keymap({
                      "Mod-z": () => undo$1(this.outerView.state, this.outerView.dispatch),
                      "Mod-y": () => redo$1(this.outerView.state, this.outerView.dispatch)
                  })]
              }),
  ...
  // nodeview_dispatchInner{
      dispatchInner(tr) {
          ...
          if (!tr.getMeta("fromOutside")) {
              let outerTr = this.outerView.state.tr, offsetMap = StepMap$1.offset(this.getPos() + 1);
  ...
  window.view = new EditorView(document.querySelector("#editor"), {
      state: EditorState.create({
          doc: DOMParser$1.fromSchema(footnoteSchema).parse(document.querySelector("#content")),
